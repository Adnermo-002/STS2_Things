...
Steam Workshop 的单个条目只能保存一套 `content/`。从 `1.7.0` 开始，所有玩家下载同一套：
...
2. `UnifiedBootstrap` 检查当前 `AbstractModel.ModifyDamageMultiplicative` 签名。
3. 五参数选择内嵌 `v107.1.dll`；六参数选择内嵌 `v109.dll`。
...
V109 直接调用官方 `ModManager.AssociateAssemblyWithMod`。随后
