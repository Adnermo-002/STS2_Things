...
OUT = HERE / "03_bind_review" / "weighted_motion_v01"
...
def load_assets() -> tuple[list[dict], dict[str, np.ndarray], dict[str, np.ndarray], np.ndarray]:
    ...
    underlays = {name: load_canvas(spec["underpaint"]) for name, spec in CHAINS.items()}
    underlays["torso"] = load_canvas(UNDER / "torso_underpaint.png")
    master = load_canvas(MASTER)
    return manifest["parts"], parts, underlays, master
...
def semantic_matrix(semantic: str, p: dict, root: np.ndarray, chain_mats: dict[str, dict[str, np.ndarray]]) -> np.ndarray:
    ...
    if semantic == "near_shoulder_plate":
        return root @ chain_mats["near_arm"]["plate"]
    ...
    if semantic == "far_shoulder_plate":
        return root @ chain_mats["far_arm"]["plate"]
    ...
    pivot_map = {
        "hooded_head": (np.asarray([PAD_X + 235.0, PAD_Y + 190.0]), p["head"]),
        "eye_glow": (np.asarray([PAD_X + 235.0, PAD_Y + 190.0]), p["head"]),
        "scarf_front": (np.asarray([PAD_X + 249.0, PAD_Y + 194.0]), p["scarf"]),
        "scarf_back": (np.asarray([PAD_X + 249.0, PAD_Y + 194.0]), p["scarf"]),
        "cape_back": (np.asarray([PAD_X + 350.0, PAD_Y + 160.0]), p["cape"]),
        "loot_sack": (np.asarray([PAD_X + 365.0, PAD_Y + 125.0]), p["sack"]),
        "bag_knot": (np.asarray([PAD_X + 365.0, PAD_Y + 105.0]), p["sack"]),
        "strap_back": (np.asarray([PAD_X + 300.0, PAD_Y + 145.0]), p["sack"] * 0.55),
        "pouch": (np.asarray([PAD_X + 339.0, PAD_Y + 254.0]), p["pouch"]),
    }
    if semantic in pivot_map:
        pivot, angle = pivot_map[semantic]
        return root @ around(pivot, angle)
    return root


def render(records: list[dict], parts: dict[str, np.ndarray], underlays: dict[str, np.ndarray], p: dict, bind: bool) -> np.ndarray:
    root = root_matrix(p)
    chain_mats = {}
    ...
    inserted = set()
    insert_at = {5: "far_leg", 8: "far_arm", 10: "torso", 13: "near_leg", 19: "near_arm"}
        ...
        if not bind:
            for threshold, name in insert_at.items():
                if z >= threshold and name not in inserted:
                    if name == "torso":
                        posed = warp_rigid(underlays[name], root)
                    else:
                        posed = warp_mesh(
                            underlays[name],
                            CHAINS[name],
...
def main() -> None:
    ...
    records, parts, underlays, master = load_assets()
    frames = []
    audits = []
    for name, overrides in CASES:
        p = parameters(overrides)
        frame = render(records, parts, underlays, p, bind=name == "bind")
    ...
    report = {
        ...
        "technique": "continuous two-bone weighted cloth underpaint + exact locked-master overlays",
