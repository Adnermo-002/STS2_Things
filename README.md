...
当前 Mod 版本：`1.6.0`。项目包含版本专用 DLL、版本专用 manifest 和共用 PCK。
...
- .NET：`9.0`
- C#：`13`
- V108 基线：`v0.108.0` / commit `58694f64`
...
```powershell
.\scripts\build.ps1 -TargetVersion v108
```
...
.\scripts\build-all.ps1 `
  ...
  -DataDirV108 "D:\Steam\steamapps\common\Slay the Spire 2\data_sts2_windows_x86_64"
```

只验证 DLL 时可追加 `-SkipPck`。构建并安装单一目标版本：

```powershell
.\scripts\build.ps1 -TargetVersion v108 -Install
```

发布工件分别位于：

- `build/v107.1/STS2_Things.json|dll|pck`
- `build/v108/STS2_Things.json|dll|pck`
